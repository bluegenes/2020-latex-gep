...
%consensus approaches and tools have emerged, 
%researchers have settled on some consensus analysis approaches for data quality control, etc.

The key advancements over the past few years have come in workflow scripting and software management. % and sharing? Role of github/open code?
...
%First, software installation via package managers (particularly conda (cite)) vastly decreased the time and effort required to try new software.
%Second, workflow languages (e.g. snakemake, nextflow, common workflow language) have matured and seen widespread uptake.
%These languages facilitate building, running, and documenting bioinformatic analyses.

%have become standardized enough/simple enough to eclipse bash scripting. 
...
For example, a GitHub repository documenting the reanalysis of the Marine Microbioal Eukaryote Transcriptome Sequencing Project uses a README alongside the code to document the workflow and digital object identifiers for data products (https://github.com/dib-lab/dib-MMETSP) \cite{johnson2019}.   
...
Snakemake, Nextflow, Common Workflow Language, and Workflow Definition Language allow the user to fully script a workflow, while websites like Galaxy and Seven Bridges Genomics offer online portals in which users build workflows. 
