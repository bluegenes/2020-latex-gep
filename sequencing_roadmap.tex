...
In this paper, we present our guide for biological sequence data analysis, developed through our own teaching, training and analysis, as well as via robust discussions with the open source community. % let's ask for thoughts explicitly?
Our main goal is to accelerate scientists conducting sequence analyses into organized workflow practices that benefit their own research while also facilitating open, reproducible, and ... analyses(FAIR?).
}
...
Sequencing data are now widely available for species across the tree of life, and new sequencing data continues to be generated at a fantastic clip/an alarming rate/etc.
...
%The widespread availability of sequencing data across the tree of life ..
%The emergence of sequencing as an affordable, standard biologial approach for studying function and diversity has created an analysis problem...

The enormity of sequencing data requires a principled approach to management, analysis, and sharing of data and insights/results/etc.
Over the past 6ish years, several papers have presented ``best" or ``good enough" practices for computational biological analyses (cite, cite, cite).
Since the last installment (cite), several important changes have greatly reduced the barrier to entry.
Many of these changes owe their origin, at least in part, to the open science movement.
...
Finally, recognition of the importance of traning .. e.g. success/importance of The Carpentries for entry-level training.

In this paper, we build on the advice and present a workflow-based approach to biological sequence analysis.
...
%tested over the past decade with workflow approaches that greatly facilitate reproducible research.
