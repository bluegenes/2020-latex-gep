...
However, while workflows are ubiquitous in bioinformatics and scientific computing in general, workflow systems designed by bioinformaticians for bioinformatic tasks are relatively new. 
...
When these processes finish, the workflow system keeps track of finished files and removes files if there was a problem in execution.
...
The minimal overhead associated with implementing these features once a workflow is specified in workflow system syntax greatly empowers the user.
...
Workflow systems are not the only option for construction and executing a workflow.
...
In our experience, it is more difficult to identify partially-completed workflow steps that produced truncated files, to rerun specific steps in a workflow, and to add additional files to a workflow when using bash scripting over workflow systems.
...
\subsection*{Explore tools that leverage computational approximations}
...
\section*{Strategies to get the most out of your data}
...
\subsection*{Project Management and Organization}
...
When you download these files, you can calculate the MD5sum size of the downloaded file, and then check that this value matches the value in the provided MD5sum file: files that are the exact same size are likely complete and uncorrupted (\textbf{Figure \ref{fig:checksum}}). 
